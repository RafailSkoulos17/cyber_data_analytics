...
def plot_against(test_df, pred_df, features, dataset):
    # axes = df.loc[start_date:end_date][features].plot(figsize=(11, 9), subplots=True)
    fig, ax = plt.subplots(len(features), 1, sharex=True)
    colors = ['b', 'r', 'g', 'c', 'm', 'y', 'k']
    for ind, feature in enumerate(features):
        ax[ind].plot(test_df[feature], colors[2*ind], label='true')
        ax[ind].plot(pred_df[feature], colors[2*ind+1], label='predicted')
        ax[ind].xaxis.set_major_locator(mdates.DayLocator([5, 10, 15, 20, 25, 30]))
        ax[ind].xaxis.set_major_formatter(mdates.DateFormatter('%d/%m'))
        ax[ind].set_ylabel(feature)
        ax[ind].grid()
...
def sliding_window_predict(df, k, w='boxcar'):
    test_df = df.loc['2017-01-04 00':'2017-04-01 00']
    shifted = df.rolling(k, win_type=w).mean().shift(1)
    plot_against(test_df, shifted.loc['2017-01-04 00':'2017-04-01 00'], ['L_T1', 'L_T2', 'L_T3'], 'naive_predictions')
...
sliding_window_predict(pd.concat([scaled_df1, scaled_df2, scaled_test_df], ignore_index=False), 24, w='triang')
