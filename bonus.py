...
def aggregate_data(data, aggr_type='card_over_day'):
    ...
    card_per_month = data[['card_id', 'creation_month']]
    card_per_month['agg_card_month'] = 1
    card_per_month = card_per_month.groupby(['card_id', 'creation_month'], as_index=False).sum()
    data = pd.merge(data, card_per_month, how='inner')

    card_per_day = data[['card_id', 'creation_day']]
    card_per_day['agg_card_day'] = 1
    card_per_day = card_per_day.groupby(['card_id', 'creation_day'], as_index=False).sum()
    data = pd.merge(data, card_per_day, how='inner')

    currency_per_month = data[['currencycode', 'creation_month']]
    currency_per_month['currency_month'] = 1
    currency_per_month = currency_per_month.groupby(['currencycode', 'creation_month'], as_index=False).sum()
    data = pd.merge(data, currency_per_month, how='inner')

    merchant_per_month = data[['accountcode', 'creation_month']]
    merchant_per_month['merchant_month'] = 1
    mer_per_month = merchant_per_month.groupby(['accountcode', 'creation_month'], as_index=False).sum()
    data = pd.merge(data, mer_per_month, how='inner')

    data.drop(['creationdate', 'creation_month', 'creation_year', 'creation_day', 'creation_hour'], axis=1,
              inplace=True)
...
if __name__ == "__main__":
    ...
    data = aggregate_data(deepcopy(data), aggr_type='merchant_per_month')
